...
RULES: tuple[Rule, ...] = (
    ...
    Rule("account_upgrade", "Paid-task scam", "Account or tier upgrade required", "critical", 34,
         r"(?:upgrade|activate|unlock|increase).{0,30}(?:account|level|tier|vip|withdrawal limit).{0,45}(?:pay|deposit|top[ -]?up|fee)|(?:pay|deposit).{0,35}(?:upgrade|vip|higher tier)", "A paid upgrade is often another advance-fee stage.", A_PAY),
    ...
    Rule("reshipping", "Reshipping scam", "Asked to receive, repackage or forward parcels", "critical", 44,
         r"(?:receive|accept).{0,35}(?:parcel|package|shipment|goods).{0,70}(?:reship|re-ship|forward|send|new address)|(?:repack|repackage|shipping agent|package inspector|parcel agent|quality control manager)", "Reshipping jobs can make a victim an intermediary for stolen goods.", A_JOB),
    ...
    Rule("sensitive_data", "Identity harvesting", "Sensitive identity, authentication or banking data requested", "critical", 35,
         r"(?:send|share|provide|upload|tell).{0,35}(?:singpass|passport|nric|social security|bank login|bank statement|one[ -]?time password|otp|seed phrase|private key|screen sharing|anydesk)", "Early requests for secrets or identity documents can enable takeover and identity theft.", "Do not share the data or code. Contact the organization through an independently verified channel."),
    ...
    Rule("unrealistic_return", "Investment scam", "Extreme short-term return claimed", "critical", 34,
         r"(?:earn|make|profit|return|roi|yield|double).{0,35}(?:\d{2,4}\s*%|\$\s*\d{3,}|s\$\s*\d{3,}).{0,35}(?:day|daily|week|weekly|hour|overnight)|(?:\d{2,4}\s*%).{0,25}(?:return|profit|roi|daily|weekly)|double your (?:money|capital).{0,25}(?:day|week|month)", "Extreme short-term returns conflict with ordinary risk-return relationships.", A_INV),
